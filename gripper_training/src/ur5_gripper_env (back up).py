...
class Ur5GripperEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Discrete(14) #Forward,Left,Right,Up,Down
    ...
    def _reset(self):
        ...
        self.selected = box_list[random.randint(0, 2)]
        ...
        state_msg.pose.position.x = -0.5 + 0.04*random_box_position_x
        state_msg.pose.position.y = -0.2 + 0.04*random_box_position_y
    ...
    #this is an inital approach to the object based on what we see (heuristic)
            ...
            pose_target.position.z = self.coordinates[2] + 0.15 #0.12
    ...
    # distance, up/right/forward etc , Is box down or false?
            observation = [distance , relative_position, False, self.box_lays_down, self.ratio_classification]
...
#########################################-----------------------$#################################3
 ...
 #we also check the cartesian distance (if gripper has come closer to the object)
        ...
        if gripper_position[2]<1.13 and action==1:
            reward-=2000
        ...
        if action==6 and (self.ratio_classification==1 or self.ratio_classification==4):
            reward-=1000

        if action==7 and (self.ratio_classification==2 or self.ratio_classification==3):
            reward-=1000
        ...
        if action> 10 and dist_after_grasp_attempt>0.3:
            reward-=500
            print("grasp FAILED!!!")
            done= True

        if action> 10 and dist_after_grasp_attempt<0.4:
            reward+=5800
            print("grasp succeeded!!!")
            done= True


        if action> 10 and current_dist<0.15:
            reward+= 800
        # descritize distance:
        ...
        state = [distance, relative_position, did_fall, self.box_lays_down, self.ratio_classification]
