...
reg = register(
    ...
    entry_point='ur5_gripper_env_testing:Ur5GripperEnv',
...
#########################################-----------------------$#################################3
 ...
 #we also check the cartesian distance (if gripper has come closer to the object)
        ...
        if action==4 and dist_after_grasp_attempt<0.4:
            ...
            if self.box_lays_down:
                done= 'success_down'
            elif not self.box_lays_down:
                done = 'success_upright'
        #
...
#don't rotate if you are too close
    ...
    def box_condition(self):
        ...
        pitch = euler[1]
        yaw = euler[2]
        if abs(roll)>0.2 or abs(pitch)>0.2 or abs(yaw)>0.2:
            did_fall = True
